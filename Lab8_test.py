...
class Stepper:
    ...
    def __step(self, dir):
        ...
        # --- FIX: compose bits without clobbering other motors ---
        # Make a mask covering just this motor's 4 bits:
        mask = (0b1111 << self.shifter_bit_start)
        # Clear *only* this motor's bits:
        Stepper.shifter_outputs &= ~mask
        # OR in this step's 4-bit pattern at our bit-offset:
        Stepper.shifter_outputs |= (Stepper.seq[self.step_state] << self.shifter_bit_start)

        # Shift the combined byte to the 74HC595:
        self.s.shiftByte(Stepper.shifter_outputs)

        # Bookkeeping angle (deg), wrap to [0,360):
    ...
    def __rotate(self, delta):
        ...
        numSteps = int(Stepper.steps_per_degree * abs(delta))    # number of half-steps
    ...
    def goAngle(self, angle):
        """
        Non-blocking: compute shortest-path delta from current logical angle
        to the requested absolute angle (degrees), then reuse rotate(delta).
        """
        # Normalize to [0,360)
        tgt = angle % 360
        curr = self.angle % 360
